...
# 4. 死区补偿、温漂修正、数据域换算、外推监视都必须显式展示，不能藏在黑盒里。
...
def calc_switching_energy(
    ...
    """
    开关能量修正链：
    基础能量提取 -> 显式温漂修正 -> 门极电阻修正 -> 电压指数修正

    这里新增两种企业对标模式：
    1. CAE精确二维插值：直接按当前 Ipk 与 Tj 查矩阵；
    2. 标称点比例法：先查 I_nom_ref 的标称能量，再按电流比例缩放；二极管保留 Ki_frd 指数项。
    """
    ...
    if "直线比例法" in algo_type:
        ...
        e_base = e_nom * current_correction
...
def calc_pwm_conduction_losses(
    ...
    if mode == "SVPWM" and is_sic:
        cosphi_sq_term = 2.0 * active_cosphi**2 - 1.0
        sv_factor = 24.0 * active_cosphi - 2.0 * math.sqrt(3.0) * cosphi_sq_term - 3.0 * math.sqrt(3.0)
        diode_r_factor = (
            6.0 * math.pi
            - 24.0 * m_eff * active_cosphi
            + 2.0 * math.sqrt(3.0) * m_eff * cosphi_sq_term
            + 3.0 * math.sqrt(3.0) * m_eff
        ) / 24.0
        p_cond_main = (sv_factor / 24.0) * m_eff * r_main_total * i_pk_chip**2 / math.pi
        p_cond_diode = ((4.0 - m_eff * math.pi * active_cosphi) / 4.0) * (diode_model["v0"] * i_pk_chip / math.pi) + diode_r_factor * (
            r_diode_total * i_pk_chip**2 / math.pi
        )
    elif mode == "SVPWM":
    ...
    if dead_ratio > 0.0:
        ...
        elif dead_meta["current_sign"] >= 0.0:
            p_cond_main = max(0.0, p_cond_main - dead_ratio * avg_inst_main)
            p_cond_diode = p_cond_diode + dead_ratio * avg_inst_diode
        else:
...
def build_star_ccm_total_heat_table(n_sim: int, p_main_chip: float, p_diode_chip: float, p_total_chip: float) -> pd.DataFrame:
    rows = []
...
def build_icepak_heat_table(n_sim: int, p_main_chip: float, p_diode_chip: float, p_total_chip: float) -> pd.DataFrame:
    rows = []
    for leg_prefix in ("Upper", "Lower"):
        for idx in range(1, n_sim + 1):
            rows.append(
...
def build_formula_audit_df(inputs: dict, dead_meta: dict, eon_meta: dict, eoff_meta: dict, erec_meta: dict) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        [
            ...
            {"审计项": "Eon 温度策略", "当前实现": eon_meta["strategy_label"], "说明": "无论矩阵维度如何，经验温漂系数均显式生效"},
            {"审计项": "Eoff 温度策略", "当前实现": eoff_meta["strategy_label"], "说明": "二维插值与温漂修正同时保留，不再清零温漂"},
            {"审计项": "Erec 温度策略", "当前实现": erec_meta["strategy_label"], "说明": "二极管恢复能量同样保留显式温漂修正"},
...
def simulate_system(inputs: dict, tables: dict):
    ...
    for loop_idx in range(loop_count):
        ...
        cond_domain_is_arm = use_half_bridge_cond
        sw_domain_is_arm = use_half_bridge_sw
        cond_src_count_eval = float(cond_src_count if cond_domain_is_arm else 1)
        sw_src_count_eval = float(sw_src_count if sw_domain_is_arm else 1)

        # All merges are done in bare-die domain first, then scaled back to the target arm.
        p_cond_main_chip = cond_result["p_cond_main"] / cond_src_count_eval
        p_cond_diode_chip = cond_result["p_cond_diode"] / cond_src_count_eval
        p_sw_main_bare_die = p_sw_main_chip / sw_src_count_eval
        p_sw_diode_bare_die = p_sw_diode_chip / sw_src_count_eval

        p_main_chip = p_cond_main_chip + p_sw_main_bare_die
        p_diode_chip = p_cond_diode_chip + p_sw_diode_bare_die
        p_total_chip = p_cond_main_chip + p_sw_main_bare_die + p_cond_diode_chip + p_sw_diode_bare_die
    ...
    star_ccm_df = build_star_ccm_total_heat_table(bridge_chip_count, p_main_chip, p_diode_chip, p_total_chip)
    icepak_df = build_icepak_heat_table(bridge_chip_count, p_main_chip, p_diode_chip, p_total_chip)
    ...
    loss_breakdown_df["单芯总发热量 (W)"] = p_total_chip
    ...
    summary_df = pd.DataFrame(
        [
            ...
            {"指标": "半桥臂单颗总损耗", "数值": p_total_chip, "单位": "W"},
    ...
    with guide_col:
        st.markdown(
            ...
            **📌 关键工程提醒：**
            ...
            - 无论 E-I 表是一维还是多温度矩阵，经验温漂系数都会显式参与修正。
            """
        )
    with note_col:
...
with c2:
    ...
    algo_type = st.radio(
        ...
        help="CAE 精确法直接按当前 Ipk 与 Tj 查二维矩阵；标称点比例法先取 I_nom 标称点，主开关按线性缩放，二极管按 Ki_frd 指数缩放。",
    )
    if "直线比例法" in algo_type:
...
with st.expander("🧭 数据矩阵健康度与温漂诊断", expanded=False):
    ...
    st.caption("开关能量表无论是一维还是多温度矩阵，经验温漂系数都会显式参与修正，页面仅用于展示矩阵维度与当前温漂生效状态。")
...
compute_requested = st.button("🚀 执行全工况仿真计算", use_container_width=True)
...
with st.expander("🔬 查看底层应用的完整物理公式与工程约定 (长期保留，不准删)", expanded=False):
    ...
    st.latex(r"V_{CE0,SiC}=0")
    st.latex(r"sv\_factor = 24\cos\phi - 2\sqrt{3}(2\cos^2\phi - 1) - 3\sqrt{3}")
    st.latex(r"P_{cond,SiC}^{SVPWM} = \frac{sv\_factor}{24}M_{eff}R_{tot,SiC}\frac{I_{pk}^2}{\pi}")
    st.latex(r"P_{cond,BodyDiode,v0}^{SVPWM} = \left(\frac{4 - M_{eff}\pi\cos\phi}{4}\right)V_{F0}\frac{I_{pk}}{\pi}")
    st.latex(r"P_{cond,BodyDiode,R}^{SVPWM} = \left(\frac{6\pi - 24M_{eff}\cos\phi + 2\sqrt{3}M_{eff}(2\cos^2\phi - 1) + 3\sqrt{3}M_{eff}}{24}\right)R_{tot,D}\frac{I_{pk}^2}{\pi}")
    st.latex(r"P_{cond,BodyDiode}^{SVPWM} = P_{cond,BodyDiode,v0}^{SVPWM} + P_{cond,BodyDiode,R}^{SVPWM}")
    ...
    st.latex(r"P_{cond,SiC}^{'} = \max\left(P_{cond,SiC} - D_{dead}P_{dead,main}^{avg},\,0\right)")
    st.latex(r"P_{cond,BodyDiode}^{'} = P_{cond,BodyDiode} + D_{dead}P_{dead,diode}^{avg}")
    st.write("程序会同步修正有效调制系数和主开关 / 二极管导通路径重分配。")

    st.markdown("### 📔 5. 开关损耗")
    st.latex(r"E_{base,main} = E(I_{pk},T_j)")
    st.latex(r"E_{base,alt} = E(I_{nom},T_j)\cdot \left(\frac{I_{pk}}{I_{nom}}\right)^{K_i}")
    st.latex(r"E_{rec,base} = E_{rec}(I_{nom},T_j)\cdot \left(\frac{I_{pk}}{I_{nom}}\right)^{K_{i,frd}}")
    ...
    st.latex(r"P_{sw,frd} = \frac{f_{sw}}{\pi}E_{rec,adj}")
    st.write("E-I 表无论是一维还是多温度矩阵，经验温漂系数都会显式参与修正；同时支持“CAE 精确二维插值”和“标称点直线比例法”两种能量提取路径。")
    ...
    st.latex(r"P_{chip,total} = P_{cond,SiC} + P_{sw,SiC} + P_{cond,BodyDiode} + P_{sw,BodyDiode}")
