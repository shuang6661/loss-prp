...
def validate_numeric_table(df: pd.DataFrame, table_name: str, required_cols: list[str]):
    ...
    if (cleaned["Current (A)"] < 0).any():
        errors.append(f"{table_name} 的 Current (A) 不能为负数。")
    if (cleaned["Temp (℃)"] < -273.15).any():
        errors.append(f"{table_name} 的 Temp (℃) 低于绝对零度。")
    ...
    temp_count = cleaned["Temp (℃)"].nunique()
...
def safe_interp(df: pd.DataFrame, target_i: float, target_t: float, item_name: str) -> float:
    ...
    for temp, group in clean_df.groupby("Temp (℃)"):
        ...
        if len(sorted_group) >= 2:
            func = interp1d(sorted_group["Current (A)"], sorted_group[item_name], kind="linear", fill_value="extrapolate")
...
def calc_pwm_conduction_losses(
    ...
    if dead_ratio > 0.0:
        if dead_meta["current_sign"] >= 0.0:
            ...
            p_cond_diode = max(0.0, p_cond_diode - dead_ratio * inst_diode)
...
def simulate_system(inputs: dict, tables: dict):
    ...
    for loop_idx in range(loop_count):
        ...
        eon_meta = calc_switching_energy(norm_ee_m, i_pk_chip, tj_main_current, "Eon (mJ)", inputs["vdc_act"], inputs["v_ref"], inputs["kv_on"], inputs["rg_on_act"], inputs["rg_on_ref"], inputs["kron"], inputs["t_coeff_igbt"], inputs["t_ref_dp"])
        eoff_meta = calc_switching_energy(norm_ee_m, i_pk_chip, tj_main_current, "Eoff (mJ)", inputs["vdc_act"], inputs["v_ref"], inputs["kv_off"], inputs["rg_off_act"], inputs["rg_off_ref"], inputs["kroff"], inputs["t_coeff_igbt"], inputs["t_ref_dp"])
        erec_meta = calc_switching_energy(norm_ee_d, i_pk_chip, tj_diode_current, "Erec (mJ)", inputs["vdc_act"], inputs["v_ref"], inputs["kv_frd"], 1.0, 1.0, 0.0, inputs["t_coeff_frd"], inputs["t_ref_dp"])
    ...
    temp_strategy_df = pd.DataFrame(
        [
            {"对象": "Eon", "基础查表值 (mJ)": eon_meta["e_base_mj"], "生效温漂系数": eon_meta["effective_temp_coeff"], "温漂修正因子": eon_meta["temp_correction"], "电阻修正因子": eon_meta["rg_correction"], "电压修正因子": eon_meta["voltage_correction"], "最终能量 (mJ)": eon_meta["energy_mj"], "温度策略": eon_meta["strategy_label"]},
            {"对象": "Eoff", "基础查表值 (mJ)": eoff_meta["e_base_mj"], "生效温漂系数": eoff_meta["effective_temp_coeff"], "温漂修正因子": eoff_meta["temp_correction"], "电阻修正因子": eoff_meta["rg_correction"], "电压修正因子": eoff_meta["voltage_correction"], "最终能量 (mJ)": eoff_meta["energy_mj"], "温度策略": eoff_meta["strategy_label"]},
            {"对象": "Erec", "基础查表值 (mJ)": erec_meta["e_base_mj"], "生效温漂系数": erec_meta["effective_temp_coeff"], "温漂修正因子": erec_meta["temp_correction"], "电阻修正因子": erec_meta["rg_correction"], "电压修正因子": erec_meta["voltage_correction"], "最终能量 (mJ)": erec_meta["energy_mj"], "温度策略": erec_meta["strategy_label"]},
        ]
    )
...
validated_main_ei, errs, warns = validate_numeric_table(ee_main, "主开关管开关能量表", ["Temp (℃)", "Current (A)", "Eon (mJ)", "Eoff (mJ)"])
...
validated_diode_ei, errs, warns = validate_numeric_table(ee_diode, "二极管恢复能量表", ["Temp (℃)", "Current (A)", "Erec (mJ)"])
...
if result:
    ...
    p3.metric("🔴 主芯片系统总损耗", f"{result['p_main_total_system']:.1f} W")
    p4.metric("🔵 二极管系统总损耗", f"{result['p_diode_total_system']:.1f} W")
    ...
    st.caption(f"STAR-CCM+ 当前主输出按单颗总热源 Total Heat Source (W) 组织；系统级总量按 N_arm_sys = {result['n_arm_system']} 做桥臂对称缩放。")
...
with st.expander("🔬 查看底层应用的完整物理公式与工程约定 (长期保留，不准删)", expanded=False):
    ...
    st.markdown("### 📔 5. 开关损耗")
    st.latex(r"E_{adj} = E_{nom}(I_{pk},T_j)\cdot \left(\frac{R_{g,act}}{R_{g,ref}}\right)^{K_r}\cdot \left(\frac{V_{dc}}{V_{nom}}\right)^{K_v}")
    ...
    st.write("E-I 表一旦具备多个温度维度，经验温漂系数就会被自动锁死为 0。")
